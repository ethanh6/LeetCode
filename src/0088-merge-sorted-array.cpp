...
[2,5,6].The result of the merge is [ _1_ , _2_ ,2, _3_ ,5,6] with the underlined
...
merge is [1].Note that because m = 0, there are no elements in nums1. The 0 is
...
public:
  void merge(vector<int> &nums1, int m, vector<int> &nums2, int n) {
    ...
    // edge case: fill the rest of nums2 into nums1 in the front
    while (p2 >= 0) {
      nums1[pos--] = nums2[p2--];
    }
  }
...
TEST_CASE("merge-sorted-array", "[0088]") {
  Solution sol;
  CHECK(true);
}
