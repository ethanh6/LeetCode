...
type. In this case, the input will be given as a signed integer type. It should
...
TEST_CASE("number-of-1-bits", "[0191]") {
  Solution sol;
  uint32_t n = 128;
  int output = 1;
  CHECK(sol.hammingWeight(n) == output);
}

TEST_CASE("number-of-1-bits - 1", "[0191]") {
  Solution sol;
  uint32_t n = pow(2, 32) - 2;
  int output = 31;
  CHECK(sol.hammingWeight(n) == output);
}
